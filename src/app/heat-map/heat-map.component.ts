...
export class HeatMapComponent implements OnInit, OnDestroy {
  ...
  constructor (private appService: AppService,
    ...
    this.zoomUpdateSubscription = this._mapService.zoomChange.subscribe(
      (zoomChange) => {
        ...
        this.tweetLayer.transition().duration(this.zoomSettings.duration)
        .ease(this.zoomSettings.ease)
        .attr('transform',
              //'translate(scale('+this.zoomSettings.zoomLevel+'))');
              'translate('+ this.width/2 + ','+ this.height/2  +')scale('+this.zoomSettings.zoomLevel+')translate('+ -x + ',' + -y +')');
  ...
  convertCityToIndexedArray(){
    var i = 0;
    for(let cityIndex in this.cities[0]){
      //this.indexedCities[i]=(this.cities[0][cityIndex]);
      this.indexedCities.push(this.cities[0][cityIndex]);
      i++;
    }
    //console.log(this.indexedCities);
  }

  getRandomLatLon(){
    // var index = Math.floor(Math.random() * 661);
    var index = Math.floor(Math.random() * 51);
    //console.log(this.indexedCities[index]);
    return [this.indexedCities[index].lon, this.indexedCities[index].lat];
  }
...
/*
  ...
  updateDots(tweets) {
   ...
   this.tweetLayer.selectAll('circle')
  ...
  drawStates(heatMapComponentScope, d):void {
    var statesMapDataPromise = this._mapService.getStatesMapData(d.properties.iso_a2);
    statesMapDataPromise .then(requestResult => {
      var statesMapData = requestResult.json();
      var cleanedStatesMapData = topojson.feature(statesMapData,statesMapData.objects.provinces).features;
      //console.log(statesMapData);
      //console.log(cleanedStatesMapData);
      //console.log(heatMapComponentScope);

      // heatMapComponentScope.g.selectAll('circle')
      // .data(cleanedStatesMapData).enter()
      // .append('circle')
      //   .attr('cx', function(d) {
      //     return 200;
      //   })
      ...
      heatMapComponentScope.g.selectAll(".province").remove();
      heatMapComponentScope.g
          .selectAll('.province')
          .data(cleanedStatesMapData)
          .enter().append('path')
          //.attr('d', heatMapComponentScope.path)
          .attr('class', 'province')
          .style('stroke', '#d5d8db')
          ...
          .attr("d",function (t) {
            return heatMapComponentScope.path(t);
          });
      
      /*heatMapComponentScope.g.selectAll(".state-name").remove();
      heatMapComponentScope.g.selectAll(".state-name")
                        .data(cleanedStatesMapData)
                        .enter()
                        .append("text")
                        .attr('x', function(d) {
  ...
  countryClicked(heatMapComponentScope, d):void {
    ...
    if (d && heatMapComponentScope.centered !== d){
      ...
      //console.log(d);
      heatMapComponentScope.showCountryName(heatMapComponentScope, d.properties.name, x, y );
      //heatMapComponentScope.showCountryName(heatMapComponentScope, d, d.properties.iso_a2, d.properties.name, x, y );
      //console.log(x+','+y);
    } else {
